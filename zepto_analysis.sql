...
SELECT COUNT(DISTINCT sku_id) FROM zepto;

SELECT sku_id, name
FROM zepto
WHERE name ILIKE '%Cherry Blossom%';
...
SELECT category,
SUM(discountedSellingPrice * availaleQuantity) AS total_revenue
FROM zepto
GROUP BY category
...
SELECT category,
SUM(weightsInGms * availaleQuantity) AS total_weight
FROM zepto 
GROUP BY category
